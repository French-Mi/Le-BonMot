...
const isFlashcardAnswered = ref(false);
...
const quizProgress = computed(() => ({ current: appStore.currentQuestionIndex, total: appStore.initialQuizWordCount }));
...
watch(() => appStore.currentQuestionIndex, async () => {
    ...
    isFlashcardAnswered.value = false;
...
function nextQuestion() { appStore.nextQuestion(); }
...
function handleFlashcardFeedback(feedback) {
  if (isFlashcardAnswered.value || !currentWord.value) return;
  isFlashcardAnswered.value = true;
    ...
    <div v-if="!isQuizFinished && currentWord" >
        <div class="mb-4">
            <div class="flex justify-between mb-1">
                ...
                <span class="text-sm font-medium text-blue-700">{{ quizProgress.current + 1 > quizProgress.total ? quizProgress.total : quizProgress.current + 1 }} / {{ quizProgress.total }}</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-2.5">
                <div class="bg-blue-600 h-2.5 rounded-full" :style="{ width: (quizProgress.total > 0 ? ((quizProgress.current + 1) / quizProgress.total) * 100 : 0) + '%' }"></div>
            </div>
        </div>

        <div v-if="currentQuizType === 'flashcards'" class="flex flex-col">
          <div class="flex justify-between items-center mb-1"><h3 class="text-lg font-semibold text-slate-700">Karteikarten</h3><div class="text-xs"><span class="text-green-600 mr-2">Sicher: {{ sureCount }}</span><span class="text-orange-500 mr-2">Unsicher: {{ unsureCount }}</span><span class="text-red-500">Ahnungslos: {{ noIdeaCount }}</span></div></div>
          <div id="flashcard" class="flashcard" @click="flipCard">
            <div v-if="!isCardFlipped"><div class="text-3xl font-bold text-sky-800">{{ appStore.currentQuizDirection === 'frToDe' ? currentWord.french : currentWord.german }}<button v-if="appStore.currentQuizDirection === 'frToDe'" @click.stop="speak(currentWord.french, $event)" class="speaker-btn" v-html="speakerIconSvg"></button></div><p class="text-gray-500 mt-4 text-sm">(Klicken zum Umdrehen)</p></div>
            <div v-else><div class="text-2xl text-gray-700">{{ appStore.currentQuizDirection === 'frToDe' ? currentWord.german : currentWord.french }}<button v-if="appStore.currentQuizDirection === 'deToFr'" @click.stop="speak(currentWord.french, $event)" class="speaker-btn" v-html="speakerIconSvg"></button></div></div>
          </div>
          <div v-if="isCardFlipped" class="mt-4 text-center"><button v-if="currentWord.exampleFrench" @click.stop="toggleExample" class="btn-example">{{ isExampleVisible ? 'Verbergen' : 'Beispiel' }}</button></div>
          <div v-if="isExampleVisible && isCardFlipped" class="example-box"><p><strong>FR:</strong> {{ currentWord.exampleFrench }} <button @click.stop="speak(currentWord.exampleFrench, $event)" class="speaker-btn" v-html="speakerIconSvg"></button></p><p><strong>DE:</strong> {{ currentWord.exampleGerman }}</p></div>
          <div v-if="isCardFlipped" class="mt-4 space-y-2">
            <p class="text-center text-gray-600 mb-2">Wie gut wusstest du es?</p>
            <div class="grid grid-cols-3 gap-2">
              <button @click="handleFlashcardFeedback('noIdea')" :disabled="isFlashcardAnswered" class="btn-feedback bg-red-500 hover:bg-red-600">Ahnungslos</button>
              <button @click="handleFlashcardFeedback('unsure')" :disabled="isFlashcardAnswered" class="btn-feedback bg-orange-500 hover:bg-orange-600">Unsicher</button>
              <button @click="handleFlashcardFeedback('sure')" :disabled="isFlashcardAnswered" class="btn-feedback bg-green-500 hover:bg-green-600">Sicher</button>
            </div>
          </div>
        </div>
...
.btn-feedback:disabled { opacity: 0.6; cursor: not-allowed; }
